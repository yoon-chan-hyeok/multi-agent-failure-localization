...
| 비교 기준 | 실행 기록 전체를 한 번에 판정하는 Direct |
...
주요 결과는 정답이나 실패 라벨을 사용하지 않고 과업 정보만으로 성공 조건을 만든 설정입니다. 정답을 참고한 결과는 방법의 상한을 확인하기 위한 참고값으로만 제시했습니다. 이 실험에서는 로그를 더 잘 나누는 것보다 실행 전에 성공 조건을 분명히 정하는 방식이 정확한 단계 추적에 더 잘 맞았습니다.

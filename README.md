...
**여러 AI agent가 함께 수행한 작업이 실패했을 때, 책임 agent와 다시 고쳐야 할 정확한 step을 찾는 평가 방법**
...
처음에는 긴 trace 자체가 문제라고 보고 fixed·adaptive chunk, top-k reread, multi-view와 pairwise reranking을 실험했습니다. 작은 chunk는 원인과 이후 맥락을 분리했고, 큰 chunk는 다시 long-context 문제가 됐습니다. Agent 선택은 일부 조건에서 나아졌지만 exact step은 안정적으로 개선되지 않았습니다.

그래서 문제를 "로그를 어떻게 나눌 것인가"에서 "이 과업이 성공하려면 무엇을 끝까지 지켜야 하는가"로 바꿨습니다. TSR-Loc은 task 해석과 trace localization을 두 단계로 나눕니다. Requirement compiler는 trace를 보기 전에 성공 조건을 작성하고, localizer는 그 기준으로 trace를 시간순으로 검사해 responsible agent와 earliest unrecovered step을 반환합니다.
...
### 왜 evaluator도 다시 검토했는가

방법만 바꾸고 평가 기준을 그대로 믿지 않았습니다. 결과를 감사하면서 복구된 오류를 decisive failure로 고르거나, 뒤에서 나타난 증상을 원인으로 고르거나, agent와 step 번호의 의미가 섞이는 사례를 확인했습니다. Indexing과 판정 규칙을 수정하고, 최종 실패로 이어진 가장 이른 미복구 지점을 같은 기준으로 비교했습니다.
...
- 메인 결과는 정답 step을 입력으로 주지 않는 task-only / No-GT 조건입니다.
